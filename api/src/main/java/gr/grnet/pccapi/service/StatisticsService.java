...
public class StatisticsService {
    ...
    public StatisticsDto setPrefixStatistics(
            String prefix, StatisticsRequestDto statisticsDto) {
        try {
            boolean update = false;
            if (!prefixRepository.existsByName(prefix)) {
                throw new NotFoundException("Prefix with name {" + prefix + "} does not exist");
            }
            if (statisticsRepository.getPrefixByID(prefix)) {
                update = true;
            }
            if ((statisticsDto.unresolvableCount
                    + statisticsDto.resolvableCount
                    + statisticsDto.uncheckedCount)
                    != statisticsDto.handlesCount) {
                throw new Exception("Not matching statistics numbers");
            }
            if (!update) {
